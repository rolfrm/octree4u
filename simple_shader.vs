...
void main(){
     ...
     vec3 vertex = vec3((p.x - p.z) * ang, p.y + (p.x + p.z) * ang, (p.y * - ang + (p.x + p.z) * ang) * 0.001 + 0.1);
     // a 0 -a 0
     // a 1 a 0
     // a -a a 0
     ...
     p1 += orig_position;
     vec3 orig_vertex = vec3((p1.x - p1.z) * ang, p1.y + (p1.x + p1.z) * ang, (p1.y * - ang + (p1.x + p1.z) * ang) * 0.001 + 0.1);
